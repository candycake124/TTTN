...
///***Phần Login
	...
	{
		$sql='SELECT * FROM `cthoadonnhap` WHERE id_hd = "'.$id.'"';
		$this->ketqua=$this->bienketnoi->query($sql);
		if ($this->ketqua->num_rows==0) {
			$data = 0;
		}else{
...
//KIỂM TRA NHÂN VIÊN CHƯA CÓ USER
	...
	/// Thêm Sản Phẩm
	public function addsanpham($b1,$b2,$b3,$b4,$b5,$b6,$b7)
	{
		$sql="insert into sanpham (tensp,id_loaisp,giaban,gianhap,soluong,hinhanh,mota) VALUES ('".$b1."','".$b2."','".$b3."','".$b4."','".$b5."','".$b6."','".$b7."')";
		$this->bienketnoi->query($sql);
	}
	// up date số lượng
